...
class UserAdmin(UserAdmin):
    ordering = ('nif', )
    list_display = (
        'id', 'name', 'nif', 'user_type', 'accepted_terms', 'read_terms',
    ...
    list_display_links = ('id', 'nif')
    fieldsets = (
        (None, {'fields': ('name', 'nif', 'user_type', 'password')}),
        ('Permissions', {'fields': ('groups', 'user_permissions')}),
        ('Roles', {'fields': ('is_staff', 'is_superuser', 'is_active')}),
        ('Additional', {'fields': (
            'accepted_terms', 'read_terms'
        )}),
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide', ),
            'fields': (
                'name', 'nif', 'user_type', 'password1', 'password2'
            )
        }),
    )
...
class LogEntryAdmin(admin.ModelAdmin):
    ...
    search_fields = ('user__userid', 'object_repr', 'change_message')
